...
class HomeViewModel : ViewModel(), RefreshesViewModel {
    ...
    fun leaveOrDeleteRoom(room: UserRoom) {
        ...
        viewModelScope.launch {
            try {
                if (room.isAdmin) {
                    // App Check is configured in TaskSandwichApplication and automatically
                    // attaches the token to every Cloud Function call — no need to manually
                    // fetch and pass a token. Sending only the required groupId.
                    val data = hashMapOf("groupId" to room.groupId)
                ...
                // Log full details to help diagnose the error
                if (e is com.google.firebase.functions.FirebaseFunctionsException) {
                    Log.e("LeaveOrDelete", "Functions error — code: ${e.code}, message: ${e.message}, details: ${e.details}")
                    _uiState.update { it.copy(error = "Error (${e.code}): ${e.message}") }
                } else {
                    Log.e("LeaveOrDelete", "Exception type: ${e::class.simpleName}, message: ${e.message}")
